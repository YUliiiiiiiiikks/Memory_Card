...
from PyQt5.QtWidgets import QApplication, QMessageBox, QButtonGroup, QPushButton, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QRadioButton, QGroupBox
...
class Question():
    def __init__(self, q, r_ans, w_ans1, w_ans2,w_ans3):
        ...
        self.w_ans3 = w_ans3
spisok = list()
serf = Question('Как тебя зовут?','вася1','вася2','вася3','вася4')
spisok.append(serf)
serf = Question('Сколько лет В.В.Путину?','74','70','56','79')
spisok.append(serf)
serf = Question('Как на русском языке будет pink?','Розовый','Белый','Чёрный','Жёлтый')
spisok.append(serf)
serf = Question('Какие глаза у бегемота?','голубые','зелёные','золотые','карие')
spisok.append(serf)
serf = Question('Сколько будет 2+2?','4','7','9','5')
spisok.append(serf)
serf = Question('Сколько будет 4*6?','24','36','42','10')
spisok.append(serf)
serf = Question('Сколько будет 49-6?','43','43','55','41')
spisok.append(serf)
z = 0
app= QApplication([])
window = QWidget()
